...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public $bobot_ujian_pertama = 100;

    public $bobot_remedial = 0;
    ...
    public function mount($id): void
    {
        if ($id && $id !== 'add') {
            ...
            $this->bobot_ujian_pertama = $jenis->bobot_ujian_pertama;
            $this->bobot_remedial = $jenis->bobot_remedial;
    ...
    public function save()
    {
        $payload = $this->validate([
            ...
            'bobot_ujian_pertama' => ['required', 'numeric', 'min:0', 'max:100', 'decimal:0,2'],
            'bobot_remedial' => ['required', 'numeric', 'min:0', 'max:100', 'decimal:0,2'],
            ...
            'bobot_ujian_pertama.required' => 'Bobot ujian pertama wajib diisi.',
            'bobot_remedial.required' => 'Bobot ujian kedua/remedial wajib diisi.',
        ...
        if (
            $payload['sumber_nilai'] === 'cbt'
            && (int) round((float) $payload['bobot_ujian_pertama'] * 100)
                + (int) round((float) $payload['bobot_remedial'] * 100) !== 10000
        ) {
            $this->addError('bobot_remedial', 'Total bobot ujian pertama dan remedial wajib tepat 100%.');
...
<form wire:submit="save">
    ...
    <div class="row">
        <div class="col-sm-12 col-lg-12">
            <div class="card">
                <div class="card-header"><h4>{{ $edit_id ? 'Edit Jenis Kegiatan' : 'Tambah Jenis Kegiatan' }}</h4></div>
                <div class="card-body">
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        @if ($sumber_nilai === 'cbt')
                            <div class="col-md-4 mb-3">
                                <label class="form-label">Bobot Ujian Pertama (%)</label>
                                <input type="number" min="0" max="100" step="0.01" class="form-control"
                                    wire:model="bobot_ujian_pertama">
                                @error('bobot_ujian_pertama') <div class="text-sm text-danger">{{ $message }}</div> @enderror
                            </div>
                            <div class="col-md-4 mb-3">
                                <label class="form-label">Bobot Ujian Kedua/Remedial (%)</label>
                                <input type="number" min="0" max="100" step="0.01" class="form-control"
                                    wire:model="bobot_remedial">
                                <div class="form-text">Kedua bobot dipakai hanya untuk peserta remedial dan wajib berjumlah 100%.</div>
                                @error('bobot_remedial') <div class="text-sm text-danger">{{ $message }}</div> @enderror
                            </div>
                        @endif
                    </div>
                    <div class="mb-3">
                    ...
                    @else
                        <div class="table-responsive">
                            <table class="table table-sm align-middle mb-0">
                                ...
                                <tbody>
                                    @foreach ($standar as $index => $baris)
                                        <tr wire:key="standar-{{ $index }}">
                                            <td>
                                                <input type="text" class="form-control form-control-sm"
                                                    wire:model="standar.{{ $index }}.nama"
                                                    placeholder="Contoh: Keaktifan">
                                                @error("standar.$index.nama") <div class="text-sm text-danger">{{ $message }}</div> @enderror
                                            </td>
                                            <td>
                                                <input type="number" min="0" step="0.01" class="form-control form-control-sm"
                                                    wire:model.live.debounce.400ms="standar.{{ $index }}.nilai_min">
                                                @error("standar.$index.nilai_min") <div class="text-sm text-danger">{{ $message }}</div> @enderror
                                            </td>
                                            <td>
                                                <input type="number" min="0.01" step="0.01" class="form-control form-control-sm"
                                                    wire:model.live.debounce.400ms="standar.{{ $index }}.nilai_maks">
                                                @error("standar.$index.nilai_maks") <div class="text-sm text-danger">{{ $message }}</div> @enderror
                                            </td>
                                            <td>
                                                <input type="number" class="form-control form-control-sm"
                                                    wire:model="standar.{{ $index }}.urutan">
                                                @error("standar.$index.urutan") <div class="text-sm text-danger">{{ $message }}</div> @enderror
                                            </td>
                                            <td>
                                                <select class="form-select form-select-sm"
                                                    wire:model="standar.{{ $index }}.status">
                                                    <option value="aktif">Aktif</option>
                                                    <option value="nonaktif">Nonaktif</option>
                                                </select>
                                                @error("standar.$index.status") <div class="text-sm text-danger">{{ $message }}</div> @enderror
                                            </td>
                                            <td class="text-end">
                                                @if (empty($baris['pernah_digunakan']))
                                                    <button type="button" class="btn btn-danger btn-sm"
                                                        wire:click="removeStandar({{ $index }})"
                        ...
                        <div class="row g-2 mt-3">
                            <div class="col-sm-4">
                                <div class="border rounded p-3 h-100">
                                    ...
                                    <div class="fs-5 fw-semibold">{{ $standarAktif->count() }} dari {{ count($standar) }}</div>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="border rounded p-3 h-100">
